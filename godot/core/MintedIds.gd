class_name MintedIds
# THE GENERATED CATALOGUE, BAKED.
#
# Minting a cube costs real time, so asking "does this already exist" against
# the ranked game would be minutes of work on a desktop and the better part of
# an hour on a phone. The identities are computed once and pasted in as
# eight-character ids, in level order, so a hit can name the cube it collided
# with.
#
# It is worth being exact about what this therefore covers: the ranked
# catalogue, the authored cubes, and this player's own creations. It cannot know
# what somebody else built on their own device. Cross-player uniqueness is not a
# harder version of this problem, it is a different one, and it needs a backend
# to answer.
#
# BROKEN INTO LINES ONLY BECAUSE A 16560-CHARACTER LITERAL IS NOT A DIFF ANYBODY
# CAN READ. They are concatenated on first use and never again.
...
# id -> the level number that already is that cube.
static func index() -> Dictionary:
	if not _INDEX.empty():
		return _INDEX
	var a := all()
	var i := 0
	while i + WIDTH <= a.length():
		var id := a.substr(i, WIDTH)
