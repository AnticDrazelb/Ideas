...
function layout(){
  ...
  /* THE FRAME IS PART OF THE BOARD'S FOOTPRINT.

     The bezel is drawn outside the tiles, so a board sized to the last
     available pixel puts its frame off the edge of the screen — which is
     what happened the moment one existed. Its thickness is a fraction of the
     tile, so this is one equation rather than a constant: the tiles give back
     exactly what the frame takes, and every viewport keeps the margin the fit
     test measures. The attract cube has no frame and gives back nothing. */
  if(!demo) side = side / (1 + 2*(FRAME_BW + FRAME_PAD)/N);
  S = side / N;
...
/* DEPTH IS SIGNAL STRENGTH.

   The bevels and the cast shadows are gone — a schematic is drawn with a
   plotter and a plotter has no light source. But depth cannot go with them,
   because depth is not decoration here: it is the ONLY thing on the flat view
   that says a collapse happened. Two traces three cells apart in depth sit
   side by side on screen, and if they render identically the player never
   sees the one idea the whole game is built on.

   So it moves into the channel this palette already speaks: a far run is DIM
   AND DESATURATED — a weak signal, barely powered — and a near one is bright
   and fully saturated, carrying current. Same information, no lighting model,
   nothing drawn that a pen could not draw. Colour is data.

   The chroma term is applied around the colour's own luminance, so pulling
   the saturation out never changes how BRIGHT a texel is — which means the
   deck/bedrock band is untouched by it and enforceBands never hears about
   this at all. */
function tileRGB(t, d, lightMul){
  ...
  var r = (a[0] + (b[0]-a[0])*near) * lm,
      g = (a[1] + (b[1]-a[1])*near) * lm,
      bl = (a[2] + (b[2]-a[2])*near) * lm;
  var sat = 0.40 + 0.60*near;
  var L = 0.2126*r + 0.7152*g + 0.0722*bl;
  return [L + (r - L)*sat, L + (g - L)*sat, L + (bl - L)*sat];
}
...
/* ============================================================
   SCHEMATIC MATERIALS — the flat board stops being a floor.

   Every one of these is drawn inside the 1.4:1 clamp the band contract
   allows (see TEX_LO), so none of them is a bright line on a dark substrate.
   What makes them read as circuitry rather than masonry is that every edge
   in them is ORTHOGONAL and every feature is on a pitch: runs, pads, vias,
   registration ticks. Brick is irregular by nature and a laid course reads
   as a made-by-hand thing; a bus is regular by necessity and reads as a
   made-by-machine one. Same luminance, opposite sentence.

   The per-stone relief pass then differentiates these as height, which is
   what turns a drawn trace into a raised one — copper standing proud of its
   substrate, lit from the same corner as everything else.               */

/* LIVE SURFACE — a bus. Runs on a four-texel pitch with a dark groove down
   one side of each, pads where they cross, and a via bored through some of
   the pads. It is the deck of a thing that is carrying current. */
function patBus(rng, pitch){
  var a = texNew(0.94), x, y, i;
  var rows = [], cols = [];
  for(i = pitch >> 1; i < TEXN; i += pitch){ rows.push(i); cols.push(i); }
  for(i = 0; i < rows.length; i++){
    var ry = rows[i];
    for(x = 0; x < TEXN; x++){
      a[ry*TEXN + x] = 1.10;                                  /* the run */
      a[((ry+1) % TEXN)*TEXN + x] = 0.84;                     /* its groove */
    }
  }
  for(i = 0; i < cols.length; i++){
    var cx = cols[i];
    /* only some verticals, so the board is not a plaid */
    if(rng() < 0.34) continue;
    for(y = 0; y < TEXN; y++){
      a[y*TEXN + cx] = 1.10;
      a[y*TEXN + ((cx+1) % TEXN)] = 0.84;
    }
  }
  /* pads at the junctions, and a via through a third of them */
  for(i = 0; i < rows.length; i++) for(var j = 0; j < cols.length; j++){
    var px = cols[j], py = rows[i];
    if(rng() < 0.42) continue;
    for(var dy = -1; dy <= 1; dy++) for(var dx = -1; dx <= 1; dx++)
      a[(((py+dy) % TEXN + TEXN) % TEXN)*TEXN + (((px+dx) % TEXN + TEXN) % TEXN)] = 1.12;
    if(rng() < 0.45) a[py*TEXN + px] = 0.82;                  /* bored through */
  }
  return a;
}

/* LIVE SURFACE — a graph field. Finer, quieter, and the one to reach for
   when a vault should read as a measuring surface rather than a wired one:
   a fine lattice, a heavier rule every fourth line, and corner ticks. */
function patGraph(rng){
  var a = texNew(0.97), x, y;
  for(y = 0; y < TEXN; y++) for(x = 0; x < TEXN; x++){
    var maj = (x % 8 === 0) || (y % 8 === 0);
    var min = (x % 2 === 0) || (y % 2 === 0);
    var v = maj ? 1.11 : (min ? 1.02 : 0.90);
    v += (rng() - 0.5) * 0.012;
    a[y*TEXN + x] = v;
  }
  /* registration ticks in the corners: the mark that says somebody aligned
     this, which is the difference between a grid and graph paper */
  for(var i = 0; i < 3; i++){
    a[i] = 1.12; a[i*TEXN] = 1.12;
    a[(TEXN-1)*TEXN + (TEXN-1-i)] = 1.12; a[(TEXN-1-i)*TEXN + (TEXN-1)] = 1.12;
  }
  return a;
}

/* LIVE SURFACE — a plate with a seam. The quietest of the three: two panels
   butted together with a scribed joint and a row of fixings, for vaults
   where the board should recede and let the glyphs carry the screen. */
function patPlate(rng){
  var a = texNew(1.0), x, y;
  for(y = 0; y < TEXN; y++) for(x = 0; x < TEXN; x++){
    var v = 0.99 + (rng() - 0.5)*0.03;
    if(x === 7 || y === 7) v = 0.85;                          /* the scribed seam */
    else if(x === 8 || y === 8) v = 1.09;                     /* its lit shoulder */
    a[y*TEXN + x] = v;
  }
  for(x = 2; x < TEXN; x += 5){ a[2*TEXN + x] = 1.12; a[13*TEXN + x] = 0.86; }
  return a;
}

/* DEAD LATTICE — structure without power. A cold regular frame with nothing
   running in it, which is exactly what bedrock is in this game: real
   geometry that happens to be no use on this face. */
function patLattice(rng, pitch){
  var a = texNew(0.72), x, y;
  for(y = 0; y < TEXN; y++) for(x = 0; x < TEXN; x++){
    var onx = (x % pitch === 0), ony = (y % pitch === 0);
    var v = (onx || ony) ? 0.88 : 0.72;
    if(onx && ony) v = 0.96;                                  /* the node */
    a[y*TEXN + x] = v + (rng() - 0.5)*0.02;
  }
  return a;
}

/* DEAD LATTICE — hatched fill. Diagonals are the draughtsman's mark for
   "solid, do not enter", and they are the one non-orthogonal thing allowed
   in here for exactly that reason. */
function patHatch(rng){
  var a = texNew(0.74), x, y;
  for(y = 0; y < TEXN; y++) for(x = 0; x < TEXN; x++)
    a[y*TEXN + x] = (((x + y) % 4) === 0 ? 0.92 : 0.72) + (rng() - 0.5)*0.02;
  return a;
}

var TEX_KITS = [
  {deck:function(r){ return patBus(r, 5); },  rock:function(r){ return patLattice(r, 4); }},
  {deck:patGraph,                             rock:patHatch},
  {deck:patPlate,                             rock:function(r){ return patLattice(r, 5); }}
];
...
/* ============================================================
   EVERY STONE IS LAID SEPARATELY — relief taken from the material itself.

   The block-level field gives a block a top, an underside and a thickness.
   Inside it, the material was still perfectly flat: a brick was a slightly
   different shade of the brick beside it, and a cobble was a patch. One
   texel of "lit lip" is what the patterns author by hand, and at sixteen
   texels across that is all there is room for.

   THE HEIGHT MAP IS ALREADY THERE. A pattern is a field of multipliers, and
   the places it dips are the places the surface dips: mortar courses, the
   seams between cobbles, the gaps between planks. Nobody has to author a
   second map — the one that exists can be read as height, differentiated, and
   lit. That is why this works for every kit in the game, including the ones
   nobody will write until later, without a line of per-material code.

   The gradient is taken at the RENDER grid off a bilinear sample of the
   coarse pattern, which is the whole trick: the albedo stays point-sampled
   and hard-edged, while the slope it implies is four texels wide and smooth.
   So a mortar course keeps its crisp dark line and gains a lit shoulder on
   one side and a shaded one on the other, and the wall stops being a picture
   of bricks and becomes bricks.

   Light comes from up and to the left, which is where the block bevel already
   says it is. The two must agree or a block reads as lit twice from two
   places, which is the specific look of something assembled rather than
   rendered.

   It samples with WRAP, because these patterns tile — two blocks of cobble
   side by side must not grow a seam that the material does not have.

   Built once per vault, into one array per material. A frame never sees it.
   ============================================================ */
var bumpLUT = {};
function buildBump(mt){
  var pat = TEXPAT[mt], h = new Float32Array(TEXR*TEXR), x, y;
  /* bilinear lift of the coarse pattern to the render grid — the SLOPE field
     only, never the colour */
  for(y = 0; y < TEXR; y++) for(x = 0; x < TEXR; x++){
    var fx = (x + 0.5)/TEXS - 0.5, fy = (y + 0.5)/TEXS - 0.5;
    var x0 = Math.floor(fx), y0 = Math.floor(fy), tx = fx - x0, ty = fy - y0;
    var xa = ((x0 % TEXN) + TEXN) % TEXN, xb = (xa + 1) % TEXN;
    var ya = ((y0 % TEXN) + TEXN) % TEXN, yb = (ya + 1) % TEXN;
    var p00 = pat[ya*TEXN + xa], p10 = pat[ya*TEXN + xb];
    var p01 = pat[yb*TEXN + xa], p11 = pat[yb*TEXN + xb];
    h[y*TEXR + x] = (p00 + (p10-p00)*tx) + ((p01 + (p11-p01)*tx) - (p00 + (p10-p00)*tx))*ty;
  }
  var out = new Float32Array(TEXR*TEXR);
  for(y = 0; y < TEXR; y++) for(x = 0; x < TEXR; x++){
    var xm = (x + TEXR - 1) % TEXR, xp = (x + 1) % TEXR;
    var ym = (y + TEXR - 1) % TEXR, yp = (y + 1) % TEXR;
    /* a slope rising to the RIGHT faces left, and the light is on the left,
       so it brightens; a slope rising DOWNWARD faces up, and brightens too */
    var gx = h[y*TEXR + xp] - h[y*TEXR + xm];
    var gy = h[yp*TEXR + x] - h[ym*TEXR + x];
    var s = (gx*0.62 + gy*0.78) * 2.6;
    out[y*TEXR + x] = 1 + Math.max(-0.20, Math.min(0.20, s));
  }
  bumpLUT[mt] = out;
}
function texFor(t, d, lit){
  ...
  var pat = TEXPAT[mt], bump = bumpLUT[mt], base = tileRGB(mt, dq/2, lq/TEX_LIT);
  ...
  for(i = 0; i < TEXR*TEXR; i++){
    ...
    m = pat[((py2/TEXS)|0)*TEXN + ((px2/TEXS)|0)] * shadeAt(i, deck) * bump[i];
...
/* How much of a texel survives at (x,y): 1 across the middle of the block,
   ...
   pixels of staircase. */
/* 4.2 of 16 is a twenty-six percent radius — a soft-edged toy. A cut panel
   is nearly square with the corner just broken, which is what this is now.
   It is also the difference between a grid of tiles and a lattice of cells:
   the smaller the radius, the more the gaps between blocks read as a ruled
   line rather than as gutters between pebbles. */
var CORNER_R = 1.9, cornerLUT = null;
...
function blockShade(nx, ny, deck){
  ...
  var m = 1;
  /* NOT A PANEL EITHER — A CELL ON A SCHEMATIC.

     Everything that was modelling a light source is gone: the lit lip, the
     underside, the sheen, the directional rim. A plotter has no lamp.

     ONE THING SURVIVES AND IT IS NOT LIGHTING. A cell needs an edge or
     forty-nine of them in the same material merge into one sheet and the
     grid — which IS the board — stops being visible. So the outer eighth
     steps down to a fixed value, hard, with no gradient: that is a GUTTER
     between two cells, the kind of line you would rule on a drawing, not a
     surface curving away from a light. It reads as a cut, not a bead. */
  if(e < 0.085) m *= 0.84;
  return m;
}
...
/* ---------- THE VAULT THE CUBE SITS IN -----------------------------------
   The board's edge was two hairlines. A hairline is a diagram's convention:
   it says "the picture ends here" and nothing else, and the whole screen was
   therefore a rectangle of stone with no reason to be that size, floating in
   the middle of a starfield.

   A bezel does three jobs a line cannot. It gives the board a REASON to end
   where it does — the cube is held in something. It gives the composition a
   frame, so the eye is delivered to the puzzle instead of wandering the
   background. And it is the one surface in the game that is allowed to be
   made of something other than the cube, which is what stops the picture
   being a single material from edge to edge.

   IT DOES NOT TURN WITH THE CUBE, and that is deliberate. Drawn in screen
   space in both renderers, so a drag rotates the cube INSIDE a vault that
   stays put — which is the correct reading of what the gesture does, and it
   also means the frame cannot pop at the moment the flat board hands over to
   the solid.

   IT IS ONE BLIT. Everything below — the bevel, the corner hardware, the
   outer shadow, the darkening it throws onto the board — is baked into a
   sprite when the tile size changes and drawn once a frame after that. It is
   also drawn in the VAULT'S OWN bedrock colour, so it changes material with
   the cube it holds without a single extra asset.                          */
/* thickness and clearance, in TILES — layout() reads the same two numbers
   when it decides how big a tile may be, so the frame can never grow off the
   edge of a screen */
var FRAME_BW = 0.30, FRAME_PAD = 0.10;
var frameCv = null, frameS = -1, frameBand = -1;
...
function buildFrame(){
  frameS = S; frameBand = curBand;
  var board = N*S, bw = Math.max(7, S*FRAME_BW), pad = Math.max(10, S*0.42);
  var sz = board + bw*2 + pad*2;
  frameCv = document.createElement('canvas');
  frameCv.width = frameCv.height = Math.ceil(sz*DPR);
  var g = frameCv.getContext('2d');
  g.setTransform(DPR, 0, 0, DPR, 0, 0);
  var x0 = pad, y0 = pad, side = board + bw*2;
  var ro = bw*0.9, ri = bw*0.34;
  var dk = ST.rF, lt = ST.rN;
  /* the shadow the whole vault throws on the sky */
  g.save();
  g.filter = 'blur(' + (bw*0.55).toFixed(2) + 'px)';
  g.fillStyle = 'rgba(0,0,0,0.55)';
  rrect(g, x0, y0 + bw*0.22, side, side, ro); g.fill();
  g.restore();
  /* the bezel face: a vertical gradient, lighter at the top because the
     light in this game is up and to the left */
  var fg = g.createLinearGradient(0, y0, 0, y0 + side);
  fg.addColorStop(0,    'rgb(' + ((lt[0]*0.92)|0) + ',' + ((lt[1]*0.92)|0) + ',' + ((lt[2]*0.92)|0) + ')');
  fg.addColorStop(0.5,  'rgb(' + ((dk[0]*1.15)|0) + ',' + ((dk[1]*1.15)|0) + ',' + ((dk[2]*1.15)|0) + ')');
  fg.addColorStop(1,    'rgb(' + ((dk[0]*0.72)|0) + ',' + ((dk[1]*0.72)|0) + ',' + ((dk[2]*0.72)|0) + ')');
  g.save();
  rrect(g, x0, y0, side, side, ro); g.clip();
  g.fillStyle = fg; g.fillRect(x0, y0, side, side);
  /* a whisper of the same grain the frame's material has */
  g.globalAlpha = 0.10;
  for(var q = 0; q < 220; q++){
    var qx = x0 + Math.random()*side, qy = y0 + Math.random()*side;
    g.fillStyle = Math.random() < 0.5 ? '#fff' : '#000';
    g.fillRect(qx, qy, 1.4, 1.4);
  }
  g.globalAlpha = 1;
  g.restore();
  /* outer lip: lit along the top-left, lost along the bottom-right */
  g.save();
  g.lineWidth = Math.max(1.2, bw*0.13);
  g.strokeStyle = 'rgba(255,255,255,0.20)';
  rrect(g, x0 + g.lineWidth/2, y0 + g.lineWidth/2, side - g.lineWidth, side - g.lineWidth, ro);
  g.stroke();
  g.restore();
  /* the well: punch the board out of the middle, then line it with the
     shadow the bezel casts down onto the stone */
  g.save();
  rrect(g, x0 + bw, y0 + bw, board, board, ri);
  g.save(); g.clip(); g.clearRect(x0 + bw, y0 + bw, board, board); g.restore();
  g.lineWidth = bw*0.5;
  g.strokeStyle = 'rgba(0,0,0,0.45)';
  g.stroke();
  g.lineWidth = Math.max(1, bw*0.10);
  g.strokeStyle = 'rgba(0,0,0,0.70)';
  rrect(g, x0 + bw, y0 + bw, board, board, ri); g.stroke();
  g.restore();
  /* CORNER HARDWARE. Four plates, because a frame with nothing at its
     corners is a border and a frame with something at its corners is a made
     object. They are the only ornament in the game and they are deliberately
     plain — a rounded plate and two rivets. */
  var cs = bw*1.55, rv = Math.max(1.3, bw*0.14);
  for(var c = 0; c < 4; c++){
    var cx2 = x0 + (c & 1 ? side - cs : 0), cy2 = y0 + (c & 2 ? side - cs : 0);
    g.save();
    var cg = g.createLinearGradient(cx2, cy2, cx2 + cs, cy2 + cs);
    cg.addColorStop(0, 'rgb(' + ((lt[0]*1.02)|0) + ',' + ((lt[1]*1.02)|0) + ',' + ((lt[2]*1.02)|0) + ')');
    cg.addColorStop(1, 'rgb(' + ((dk[0]*0.86)|0) + ',' + ((dk[1]*0.86)|0) + ',' + ((dk[2]*0.86)|0) + ')');
    g.fillStyle = cg;
    rrect(g, cx2, cy2, cs, cs, bw*0.5); g.fill();
    g.lineWidth = 1; g.strokeStyle = 'rgba(0,0,0,0.42)'; g.stroke();
    g.fillStyle = 'rgba(255,255,255,0.22)';
    g.beginPath(); g.arc(cx2 + cs*0.31, cy2 + cs*0.31, rv, 0, 6.284); g.fill();
    g.beginPath(); g.arc(cx2 + cs*0.69, cy2 + cs*0.69, rv, 0, 6.284); g.fill();
    g.fillStyle = 'rgba(0,0,0,0.34)';
    g.beginPath(); g.arc(cx2 + cs*0.31 + rv*0.5, cy2 + cs*0.31 + rv*0.6, rv*0.72, 0, 6.284); g.fill();
    g.beginPath(); g.arc(cx2 + cs*0.69 + rv*0.5, cy2 + cs*0.69 + rv*0.6, rv*0.72, 0, 6.284); g.fill();
    g.restore();
  }
  frameCv.sz = sz;
}
...
/* ---------- IT IS A DISPLAY, AND IT SAYS SO ------------------------------
   One band of very slightly lifted scanlines crossing the board every few
   seconds. It is the cheapest possible statement that what you are looking
   at is a readout of a system rather than a room you are standing in — and
   unlike a permanent scanline overlay it costs nothing at rest, because
   outside the sweep there is nothing to draw at all.

   It is clipped to the board and it is ADDITIVE and faint, so it can never
   darken a cell toward the bedrock band. The one thing on this screen that
   is not allowed to be touched by atmosphere is the question "may I stand
   there".                                                                 */
var sweepT = 3000;
function stepSweep(dt){ sweepT += dt; }
function drawSweep(){
  if(!store.fx || !lv || demo) return;
  var period = 5200, dur = 900, t = sweepT % period;
  if(t > dur) return;
  var p = t/dur, half = N*S/2;
  var y = CY - half - S*0.5 + p*(N*S + S);
  var h = S*0.9;
  ctx.save();
  ctx.beginPath(); ctx.rect(CX - half, CY - half, N*S, N*S); ctx.clip();
  ctx.globalCompositeOperation = 'lighter';
  var g = ctx.createLinearGradient(0, y - h, 0, y + h);
  var a = (0.055 * Math.sin(p*Math.PI)).toFixed(4);
  g.addColorStop(0,   'rgba(140,220,255,0)');
  g.addColorStop(0.5, 'rgba(140,220,255,' + a + ')');
  g.addColorStop(1,   'rgba(140,220,255,0)');
  ctx.fillStyle = g;
  ctx.fillRect(CX - half, y - h, N*S, h*2);
  ctx.restore();
}
...
function stepFlyers(dt){
  for(var i = flyers.length-1; i >= 0; i--){
    ...
    if(f.t >= f.dur){
      ...
      burst(r.left+r.width*0.32, r.top+r.height/2, 10, {spd:70, life:380, r:1.8, col:'163,230,53', g:0});
    }
  }
}
function drawFlyers(){
  for(var i = 0; i < flyers.length; i++){
    ...
    halo(x, y, 24, '163,230,53', 0.5*(1-p*0.5));
...
function draw(){
  ...
  if(exitFlash > 0){
    ...
    eg.addColorStop(0, 'rgba(249,115,22,0.5)'); eg.addColorStop(1, 'rgba(249,115,22,0)');
...
function drawFlat(){
  ...
  if(nsFlat.length){
    ...
    for(i = 0; i < nsFlat.length; i++){
      ...
      var ncol = nf.kind === 2 ? S_KEY : nf.kind === 4 ? S_EXIT : '250,204,21';
...
/* THE KEY — the diamond, in emerald, turning on a slow bob. Eight facets on
   ...
   for. */
var C_KEY = [34, 224, 106], S_KEY = '163,230,53';
...
/* THE WAY OUT — a gold torus, hanging over the square and turning about its
   ...
   drawn circle. */
var C_EXIT = [255, 196, 77], S_EXIT = '249,115,22';
...
function draw3d(){
  ...
  function drawNearLinks(){
    ...
    for(var si = 0; si < specials.length; si++){
      ...
      var col = sp.kind === 2 ? S_KEY : sp.kind === 4 ? S_EXIT : '250,204,21';
  ...
  for(i = 0; i < list.length; i++){
    ...
    if(e.k === 0){
      ...
      if(xr > 0.02){
        flushWire();
        var mc = e.k === 2 ? S_KEY : e.k === 1 ? '34,211,238' : e.k === 4 ? S_EXIT : '250,204,21';
...
function tapCell(u, v){
  ...
  if(shut && keysHeld() < 1){ toast('locked — find a key'); sfx.deny(); buzz(14); denyAt(u, v, '163,230,53'); return; }
  var path = pathTo(u, v);
...
function advanceWalk(dt){
  ...
  if(di >= 0 && !(doors & (1<<di))){
    ...
    kick(5, 0, 1); shake(0.34); punch(0.030); hitstop(46); flash('250,204,21', 0.30, 520);
    burst(cx2, cy2, 26, {spd:150, life:640, r:2.4, col:'250,204,21', g:110});
    burst(cx2, cy2, 10, {kind:'chip', spd:110, life:760, r:2.0, col:'150,120,14', g:340});
    ring(cx2, cy2, {r0:S*0.2, r1:S*2.6, dur:520, col:'250,204,21', a:0.65, w:3.5});
  } else {
    sfx.step();
    ...
    ring(cx2, cy2, {r0:S*0.30, r1:S*0.62, dur:260, col:'34,211,238', a:0.30, w:2, sq:true});
    shake(0.045);
  }
  var ki = keyIndexAt(lv, pos);
  if(ki >= 0 && !(kmask & (1<<ki))){
    kmask |= (1<<ki); sfx.key(); buzz(12); toast('key'); faceSet('happy', 900);
    shake(0.20); punch(0.022); hitstop(30); flash('163,230,53', 0.26, 460);
    burst(cx2, cy2, 20, {spd:130, life:560, r:2.2, col:'163,230,53', g:20});
    burst(cx2, cy2, 8, {kind:'ember', spd:26, lift:40, life:900, r:2.0, col:'214,246,150', g:-24});
    ring(cx2, cy2, {r0:S*0.15, r1:S*1.9, dur:460, col:'163,230,53', a:0.7, w:3});
    flyKey();
  }
  ...
  if(!walking.queue.length){
    ...
    burst(cx2, cy2, 5, {kind:'ember', spd:20, lift:26, life:700, r:1.6, col:'170,240,250', g:-18});
    settle(); afterAction();
  }
...
function advanceAnim(dt){
  ...
  burst(pr[0]+S/2, pr[1]+S/2, 10, {spd:78, life:440, r:2.1, col:'170,240,250', g:60});
  revealT = 0;
  sfx.turn(); buzz(11);
  settle(); afterAction();
}
